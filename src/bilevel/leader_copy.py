...
hyperparameters ``(alpha, beta)`` that yield the best follower response:
    ...
    min_{α,β}  L_MSE(Γ*(α,β), Q_val)
    s.t.  Γ*(α,β) = argmin_Γ  L̃(Γ, α, β)_train
...
def run_follower_worker(
    ...
    *,
    pop_size:         int   = 210,    # 10 × n_var per DE guideline
    n_gen:            int   = 400,    # set from convergence_diagnostic.py
    ftol:             float = 1e-5,
    xtol:             float = 1e-6,
    period:           int   = 20,
    ...
    Uses Differential Evolution (DE) which outperforms GA on the coupled
    21-dimensional elastic-net landscape.  Early stopping fires when both
    the objective change (``ftol``) and the design-space change (``xtol``)
    fall below their thresholds over the last ``period`` generations, or
    when ``n_gen`` hard cap is reached — run convergence_diagnostic.py to
    find the right value for your surrogates.
    """
    ...
    algorithm = DE(
        pop_size=pop_size,
        sampling=LHS(),       # even coverage across all 21 dims from gen 1
        variant=de_variant,
        CR=de_cr,
        F=de_f,
        dither="vector",      # varies F per difference vector — helps on noisy surrogates
        jitter=False,
    )
...
class LeaderGA(Problem):
    ...
    def __init__(
        ...
        *,
        ...
        follower_pop_size:    int   = 210,    # 10 × n_var per DE guideline
        follower_n_gen:       int   = 400,    # set from convergence_diagnostic.py
        ...
        if self.n_jobs == 1:
            ...
            results = Parallel(n_jobs=self.n_jobs)(
                delayed(run_follower_worker)(
                    ...
                    de_variant=self.follower_de_variant,
                    de_cr=self.follower_de_cr,
                    de_f=self.follower_de_f,
    ...
    def _solve_single_follower(self, hyperparams: np.ndarray) -> np.ndarray:
        ...
        algorithm = DE(
            pop_size=self.follower_pop_size,
            sampling=LHS(),
            variant=self.follower_de_variant,
            CR=self.follower_de_cr,
            F=self.follower_de_f,
            dither="vector",
            jitter=False,
        )
        termination = DefaultSingleObjectiveTermination(
